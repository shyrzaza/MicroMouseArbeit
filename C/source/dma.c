...
unsigned int adc1Data[32] __attribute__((space(dma)));
...
void initDmaChannel4(void)
{
    ...
    DMA4STA = (__builtin_dmaoffset(&(adc1Data[0]))); // start address of DMA RAM
    DMA4PAD = (volatile unsigned int)&ADC1BUF0; // address of peripheral sfr (0x0300)
    DMA4CNT = 0; // we have 1 a2d  s/h channel for  measurement
